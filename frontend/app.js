...
function w2c(wx,wy) {
  const P=20,W=sc.width,H=sc.height,rx=bounds.maxX-bounds.minX||1,ry=bounds.maxY-bounds.minY||1;
...
async function loadItems() {
  try {
    ...
    if (allItems.length >= 2) {
      ...
      const px=(x1-x0)*.05||.03, py=(y1-y0)*.05||.03;
...
async function runSearch() {
  ...
  } catch(_){alert('Cannot reach server — is it running on :9090?');}
}
...
async function loadDocList() {
  try {
    ...
    document.getElementById('docList').innerHTML=docs.map(d=>`
      ...
      // Rebuild graph nodes for persisted docs
if (!docsLoadedIntoGraph) {
  for (const d of docs) {
    const emb16 = textToEmbedding(d.title + ' ' + d.preview);
    ...
    console.log("Insert doc node:", await res.text());
  }

  docsLoadedIntoGraph = true;
  await loadItems();
  loadHNSW();
}
  } catch(_) {}
...
document.getElementById('exportBtn').onclick = () => {
  ...
  const cleaned = {
    latencyUs: lastSearchResults.latencyUs,
    algo: lastSearchResults.algo,
    metric: lastSearchResults.metric,
    results: lastSearchResults.results.map((r, i) => ({
      rank: i + 1,
      metadata: r.metadata,
      category: r.category,
      distance: r.distance
...
document.getElementById('uploadPdfBtn').onclick = async () => {
    ...
    try {
        ...
        alert(`Loaded ${pdf.numPages} pages successfully!`);
    } catch (err) {
        console.error(err);
