...
float dotProduct(const std::vector<float>& a, const std::vector<float>& b) {
    ...
    return -dot;   // negative because lower distance = better in existing code
}

DistFn getDistFn(const std::string& m) {
    if (m == "cosine")    return cosine;
    if (m == "manhattan") return manhattan;
    if (m == "dot")       return dotProduct;
    return euclidean;
}
...
float keywordScore(const std::string& query, const std::string& text) {
    std::stringstream qss(query), tss(text);
    std::unordered_set<std::string> qwords, twords;
    std::string word;
    ...
    if (qwords.empty()) return 0.0f;

    int matches = 0;
    for (const auto& w : qwords)
        if (twords.count(w)) matches++;

    return (float)matches / qwords.size();
}
...
public:
   public:
    ...
    void saveTextDB() {
        std::ofstream file("documents.db");
        if (!file) return;

        for (const auto& [id, doc] : store) {
    ...
    for (const auto& [id, doc] : store) {
        ...
        file << "{";
        file << "\"id\":" << doc.id << ",";
        file << "\"title\":" << jS(doc.title) << ",";
        file << "\"text\":" << jS(doc.text) << ",";
        file << "\"embedding\":[";

        for (size_t i = 0; i < doc.emb.size(); i++) {
            if (i) file << ",";
            file << doc.emb[i];
        }
...
for (auto& [d, id] : raw) {
    if (!store.count(id) || d > max_dist) continue;

    auto& doc = store[id];

    float vectorScore = 1.0f - d;
    float keyword = keywordScore(queryText, doc.text);

    float hybridScore = 0.7f * vectorScore + 0.3f * keyword;

    out.push_back({1.0f - hybridScore, doc});
}
...
//  HTTP SERVER
// =====================================================================
void loadDocsFromDisk(VectorDB& db, DocumentDB& docDB, OllamaClient& ollama) {
    std::ifstream file("documents.db");
    if (!file) return;
    ...
    while (std::getline(file, line)) {
        ...
        std::string text  = line.substr(pos + 3);

       auto emb = ollama.embed(text);
...
int main() {
   VectorDB   db(DIMS);
    ...
    loadDocsFromDisk(db, docDB, ollama);
    loadDemo(db);
    ...
    svr.Post("/doc/search", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto hits = docDB.search(qEmb, question, k);
    ...
    svr.Get("/stats", [&](const httplib::Request&, httplib::Response& res) {
        ...
        ss << "{\"count\":"      << db.size()
           ...
           << ",\"metrics\":[\"euclidean\",\"cosine\",\"manhattan\",\"dot\"]}";
        res.set_content(ss.str(), "application/json");
    });
...
svr.Get("/style.css", [](const httplib::Request&, httplib::Response& res) {
    std::ifstream f("../frontend/style.css");
    if (!f.is_open()) { res.status = 404; return; }
...
svr.Get("/app.js", [](const httplib::Request&, httplib::Response& res) {
    std::ifstream f("../frontend/app.js");
    if (!f.is_open()) { res.status = 404; return; }
